...
get_uuid() {
  ...
  # Tries uuidgen, then the Linux kernel, then python3.
  # Usage: get_uuid
  if command -v uuidgen >/dev/null 2>&1; then
    ...
    echo "get_uuid: no uuid source available" >&2
    return 1
  fi
}
...
unarc() {
  ...
  case "${1}" in
    ...
    *)                echo "'${1}' cannot be extracted via unarc()" >&2; return 1 ;;
  esac
}
...
_urdabash_help() {
  ...
  echo "  get_uuid ........ Generate a random lowercase UUID"
...
_urdabash_info() {
  ...
  [[ -n ${URDABASH_LOADED_EXTENSIONS+x} ]] && extensions_status="${URDABASH_LOADED_EXTENSIONS}"
  ...
  echo "URDABASH_LOADED_EXTENSIONS .. ${extensions_status}"
